...
# tsdisplay(train_ts, main = 'Average_MSRP_spec (training set) July 2013 to March 2021', xlab = 'Time') 
...
# Fit a linear model
lin_model <- tslm(Average_MSRP_spec ~ Semi_Price_Index + WPU03THRU15_Price_Index + WPU10_Price_Index + WPU1017_Price_Index)
summary(lin_model)
...
Average_MSRP_spec <- train_ts
exogenous <- train_df[, c("Semi_Price_Index", "WPU03THRU15_Price_Index", "WPU10_Price_Index", "WPU1017_Price_Index")]
...
xreg_vars <- test_df[, c("Semi_Price_Index", "WPU03THRU15_Price_Index", "WPU10_Price_Index", "WPU1017_Price_Index")]
...
ggplot(data_forecast_lin, aes(Time)) +
  ...
  theme_minimal() + ggtitle("18 Month Forecast vs. Actual Values for Linear model with PPI Macro factors")
```
...
The Ljung-Box tests for the presence of residual autocorrelation. The test resulted in a p-value (0.8847) greater than 0.05, 
...
ggplot(data_forecast_ARIMAX, aes(Time)) +
  ...
  labs(x = "Time", y = "Average_MSRP_spec") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("18 Month Forecast vs. Actual Values for ARIMAX model with PPI Macro factors")
```
...
The Ljung-Box test resulted in a p-value (0.9413) greater than 0.05 for this model too, indicating no evidence of 
autocorrelation. 
Also when we examine the residual plot, we observe no significant spikes in the Autocorrelation Function (ACF) at any lags. 
...
ggplot(data_forecast_VAR, aes(Time)) +
  ...
  labs(x = "Time", y = "Average_MSRP_spec") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("18 Month Forecast vs. Actual Values for VAR model with PPI Macro factors")
```
...
set.seed(seedNum)
fit.enet <- train(Average_MSRP_spec~., data=train_df[, -1], method="enet", metric="RMSE", trControl=control)
print(fit.enet)
summary(fit.enet)
...
```{r FINAL1}
# Tuning algorithm #2 - Ridge model
set.seed(seedNum)
parameters <- c(seq(0.1, 2, by =0.1) ,  seq(2, 5, 0.5) , seq(5, 25, 1))
grid <- expand.grid(alpha=0, lambda = parameters)
fit.ridge2 <- train(Average_MSRP_spec~., data=train_df[, -1], method="glmnet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.ridge2)
print(fit.ridge2)
summary(fit.ridge2)
...
point_predictions <- predict(fit.ridge2, newdata = test_df)
...
# Make point predictions using the trained model on the test data
point_predictions <- predict(fit.bagcart, newdata = test_df)

# Get the estimated standard error of the residuals
residuals <- test_df$Average_MSRP_spec - point_predictions
sigma_hat <- sqrt(sum(residuals^2) / (n - length(fit.bagcart$finalModel$lambda)))
...
ggplot(data_forecast_lasso2, aes(Time)) +
  ...
  ggtitle("18 Month Forecast vs. Actual Values for Lasso Regression model with PPI Macro Factors")
```
...
ggplot(data_forecast_ridge2, aes(Time)) +
  ...
  theme_minimal() + 
  theme(plot.title = element_text(size = 12)) +
  ggtitle("18 Month Forecast vs. Actual Values for Ridge Regression model with PPI Macro Factors")
```

```{r Q27}
# Accuracy for 18 Month - Ridge Regression model
forecast::accuracy(pred_CI_ridge2$point_predictions, test_df$Average_MSRP_spec) %>% round(4)
```
...
ggplot(data_forecast_enet2, aes(Time)) +
  ...
  ggtitle("18 Month Forecast vs. Actual Values for ElasticNet Regression model with PPI Macro Factors")
```
...
ggplot(data_forecast_cart2, aes(Time)) +
  ...
  theme_minimal() + 
  theme(plot.title = element_text(size = 11)) +
  ggtitle("18 Month Forecast vs. Actual Values for Decision Tree - CART model with PPI Macro Factors")
```
...
ggplot(data_forecast_bagcart2, aes(Time)) +
  ...
  theme_minimal() + 
  theme(plot.title = element_text(size = 12)) +
  ggtitle("18 Month Forecast vs. Actual Values for Bagged CART model with PPI Macro Factors")
```
...
ggplot(data_forecast_rf2, aes(Time)) +
  ...
  theme_minimal() + 
  theme(plot.title = element_text(size = 12)) +
  ggtitle("18 Month Forecast vs. Actual Values for Random Forest model with PPI Macro Factors")
```
...
ggplot(data_forecast_gbm2, aes(Time)) +
  ...
  theme_minimal() + 
  theme(plot.title = element_text(size = 12)) +
  ggtitle("18 Month Forecast vs. Actual Values for Gradient Boosting model with PPI Macro Factors")
```
