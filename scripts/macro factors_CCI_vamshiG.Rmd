...
train_ts <- ts(train_df$CCI)
test_ts <- ts(test_df$CCI)
tsdisplay(train_ts)
```
...
The p-value (0.7398) is greater than the significance level (0.05) for CCI
...
# tsdisplay(train_ts, main = 'CCI (training set) Jan 2013 - Sep 2019', xlab = 'Time') 
...
# tsdisplay(train_ts_trans, main = 'Box-Cox Transformation CCI with 𝝀= 1.999924', xlab = 'Time') 
```
...
# tsdisplay(diff(train_ts), main = '1st Differencing of CCI', xlab = 'Time') 
...
# Fit a linear model
lin_model <- tslm(CCI ~ GDP + Metal + Iron_Ore + Inflation)
summary(lin_model)
...
xreg_vars <- test_df[, c("GDP", "Metal", "Iron_Ore", "Inflation")]
...
data_forecast_VAR <- data.frame(Time = test_df$DATE,
                   Actual = test_df$CCI,
                   Forecasted = fit_forecast_VAR$forecast$CCI)
```
...
forecast::accuracy(fit_forecast_lin, test_df$CCI) %>% round(4)
```
...
forecast::accuracy(fit_forecast_ARIMAX, test_df$CCI) %>% round(4)
```
...
```{r Q27}
# Accuracy for 12 Month - Linear model
forecast::accuracy(fit_forecast_VAR$forecast$CCI, test_df$CCI) %>% round(4)
```
...
fit.lm <- train(CCI~., data=train_df[, -1], method="lm", metric="RMSE", trControl=control)
print(fit.lm)
summary(fit.lm)
...
# Ridge (Regression)
set.seed(seedNum)
fit.ridge <- train(CCI~., data=train_df[, -1], method="ridge", metric="RMSE", trControl=control)
print(fit.ridge)
summary(fit.ridge)
...
set.seed(seedNum)
fit.lasso <- train(CCI~., data=train_df[, -1], method="lasso", metric="RMSE", trControl=control)
print(fit.lasso)
summary(fit.lasso)
...
set.seed(seedNum)
fit.enet <- train(CCI~., data=train_df[, -1], method="enet", metric="RMSE", trControl=control)
print(fit.enet)
summary(fit.enet)
...
set.seed(seedNum)
fit.cart <- train(CCI~., data=train_df[, -1], method="rpart", metric="RMSE", trControl=control)
print(fit.cart)
summary(fit.cart)
...
set.seed(seedNum)
fit.bagcart <- train(CCI~., data=train_df[, -1], method="treebag", metric="RMSE", trControl=control)
print(fit.bagcart)
summary(fit.bagcart)
...
set.seed(seedNum)
fit.rf <- train(CCI~., data=train_df[, -1], method="rf", metric="RMSE", trControl=control)
print(fit.rf)
summary(fit.rf)
...
set.seed(seedNum)
fit.gbm <- train(CCI~., data=train_df[, -1], method="xgbTree", metric="RMSE", trControl=control, verbose=F, verbosity = 0)
...
fit.ridge2 <- train(CCI~., data=train_df[, -1], method="glmnet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.ridge2)
print(fit.ridge2)
summary(fit.ridge2)
...
fit.lasso2 <- train(CCI~., data=train_df[, -1], method="glmnet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.lasso2)
print(fit.lasso2)
summary(fit.lasso2)
...
fit.enet2 <- train(CCI~., data=train_df[, -1], method="enet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.enet2)
print(fit.enet2)
summary(fit.enet2)
...
fit.cart2 <- train(CCI~., data=train_df[, -1], method="rpart", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.cart2)
print(fit.cart2)
summary(fit.cart2)
...
fit.bagcart2 <- train(CCI ~ ., data = train_df[, -1], method = "treebag",  metric = "RMSE", trControl = control, nbagg = 106)
#plot(fit.bagcart2)
print(fit.bagcart2)
...
# Perform hyperparameter tuning
fit.rf2 <- train(CCI ~ ., data = train_df[, -1], method = "rf", metric = "RMSE", trControl = control, 
...
# Perform hyperparameter tuning
fit.gbm2 <- train(CCI ~ ., data = train_df[, -1], method = "xgbTree", metric = "RMSE", trControl = control, verbose = FALSE, verbosity = 0, tuneGrid = hyperparameters)
plot(fit.gbm2)
print(fit.gbm2)
summary(fit.gbm2)
...
residuals <- test_df$CCI - point_predictions
...
forecast::accuracy(pred_CI_ridge2$point_predictions, test_df$CCI) %>% round(4)
```
...
forecast::accuracy(pred_CI_lasso2$point_predictions, test_df$CCI) %>% round(4)
```
...
forecast::accuracy(pred_CI_enet2$point_predictions, test_df$CCI) %>% round(4)
```
...
forecast::accuracy(pred_CI_cart2$point_predictions, test_df$CCI) %>% round(4)
```
...
forecast::accuracy(pred_CI_bagcart2$point_predictions, test_df$CCI) %>% round(4)
```
...
forecast::accuracy(pred_CI_rf2$point_predictions, test_df$CCI) %>% round(4)
```
...
forecast::accuracy(pred_CI_gbm2$point_predictions, test_df$CCI) %>% round(4)
```
