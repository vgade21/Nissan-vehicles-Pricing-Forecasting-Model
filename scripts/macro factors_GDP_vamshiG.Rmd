...
```{r Q1}
# loading the data
data <- read.csv("C:\\Users\\vamsh\\Documents\\Uni\\UChicago\\Summer 2023\\MSCA 34003_IP01 - Capstone II\\Project\\Data\\Macro Factors\\merged_table_final_filtered.csv")
```
...
data_3 <- subset(data_2, select = c("DATE", "CCI", "GDP", "Metal", "Iron_Ore", "Inflation"))
...
train_ts <- ts(train_df$GDP)
test_ts <- ts(test_df$GDP)
tsdisplay(train_ts)
```
...
The p-value (0.9361) is greater than the significance level (0.05) for CCI
...
# tsdisplay(diff(train_ts_trans), main = '1st Differencing of GDP', xlab = 'Time') 
# # 1st order Differencing isn't required
```

```{r Q1}
# # Augmented Dickey-Fuller Test
# adf.test(diff(train_ts_trans)) # Appears to be stationary
```

```{r Q1}
# # KPSS test
# kpss.test(diff(train_ts_trans), null = c("Level", "Trend"), lshort = TRUE) # Appears to be stationary
```
...
CCI <- train_df[,"CCI"]
Metal <- train_df[,"Metal"]
Iron_Ore <- train_df[,"Iron_Ore"]
Inflation <- train_df[,"Inflation"]

# Fit a linear model
lin_model <- tslm(GDP ~ CCI + Metal + Iron_Ore + Inflation)
summary(lin_model)
...
xreg_vars <- test_df[, c("CCI", "Metal", "Iron_Ore", "Inflation")]
xreg_vars <- as.matrix(xreg_vars[,1:4])
...
data_forecast_VAR <- data.frame(Time = test_df$DATE,
                   Actual = test_df$GDP,
                   Forecasted = fit_forecast_VAR$forecast$GDP)
```
...
ggplot(data_forecast_lin, aes(Time)) +
  ...
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for Linear model")
```

```{r Q27}
# Accuracy for 12 Month - Linear model
forecast::accuracy(fit_forecast_lin, test_df$GDP) %>% round(4)
```
...
ggplot(data_forecast_ARIMAX, aes(Time)) +
  ...
  labs(x = "Time", y = "GDP") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for ARIMAX model")
```

```{r Q27}
# Accuracy for 12 Month - ARIMAX model
forecast::accuracy(fit_forecast_ARIMAX, test_df$GDP) %>% round(4)
```
...
ggplot(data_forecast_VAR, aes(Time)) +
  ...
  labs(x = "Time", y = "GDP") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for VAR model")
```

```{r Q27}
# Accuracy for 12 Month - Linear model
forecast::accuracy(fit_forecast_VAR$forecast$GDP, test_df$GDP) %>% round(4)
```
...
fit.lm <- train(GDP~., data=train_df[, -1], method="lm", metric="RMSE", trControl=control)
print(fit.lm)
summary(fit.lm)
...
# Ridge (Regression)
set.seed(seedNum)
fit.ridge <- train(GDP~., data=train_df[, -1], method="ridge", metric="RMSE", trControl=control)
print(fit.ridge)
summary(fit.ridge)
...
set.seed(seedNum)
fit.lasso <- train(GDP~., data=train_df[, -1], method="lasso", metric="RMSE", trControl=control)
print(fit.lasso)
summary(fit.lasso)
...
set.seed(seedNum)
fit.enet <- train(GDP~., data=train_df[, -1], method="enet", metric="RMSE", trControl=control)
print(fit.enet)
summary(fit.enet)
...
set.seed(seedNum)
fit.cart <- train(GDP~., data=train_df[, -1], method="rpart", metric="RMSE", trControl=control)
print(fit.cart)
summary(fit.cart)
...
set.seed(seedNum)
fit.bagcart <- train(GDP~., data=train_df[, -1], method="treebag", metric="RMSE", trControl=control)
print(fit.bagcart)
summary(fit.bagcart)
...
set.seed(seedNum)
fit.rf <- train(GDP~., data=train_df[, -1], method="rf", metric="RMSE", trControl=control)
print(fit.rf)
summary(fit.rf)
...
set.seed(seedNum)
fit.gbm <- train(GDP~., data=train_df[, -1], method="xgbTree", metric="RMSE", trControl=control, verbose=F, verbosity = 0)
...
fit.ridge2 <- train(Inflation~., data=train_df[, -1], method="glmnet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.ridge2)
print(fit.ridge2)
summary(fit.ridge2)
...
fit.lasso2 <- train(GDP~., data=train_df[, -1], method="glmnet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.lasso2)
print(fit.lasso2)
summary(fit.lasso2)
...
fit.enet2 <- train(GDP~., data=train_df[, -1], method="enet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.enet2)
print(fit.enet2)
summary(fit.enet2)
...
fit.cart2 <- train(GDP~., data=train_df[, -1], method="rpart", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.cart2)
print(fit.cart2)
summary(fit.cart2)
...
fit.bagcart2 <- train(GDP ~ ., data = train_df[, -1], method = "treebag",  metric = "RMSE", trControl = control, nbagg = 106)
#plot(fit.bagcart2)
print(fit.bagcart2)
...
# Perform hyperparameter tuning
fit.rf2 <- train(GDP ~ ., data = train_df[, -1], method = "rf", metric = "RMSE", trControl = control, 
...
# Perform hyperparameter tuning
fit.gbm2 <- train(GDP ~ ., data = train_df[, -1], method = "xgbTree", metric = "RMSE", trControl = control, verbose = FALSE, verbosity = 0, tuneGrid = hyperparameters)
plot(fit.gbm2)
print(fit.gbm2)
summary(fit.gbm2)
...
residuals <- test_df$GDP - point_predictions
...
# Make point predictions using the trained model on the test data
point_predictions <- predict(fit.gbm2, newdata = test_df)

# Get the estimated standard error of the residuals
residuals <- test_df$GDP - point_predictions
sigma_hat <- sqrt(sum(residuals^2) / (n - length(fit.gbm2$finalModel$lambda)))
...
ggplot(data_forecast_ridge2, aes(Time)) +
  ...
  labs(x = "Time", y = "GDP") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for Ridge Regression model")
```

```{r Q27}
# Accuracy for 12 Month - Ridge Regression model
forecast::accuracy(pred_CI_ridge2$point_predictions, test_df$GDP) %>% round(4)
```
...
ggplot(data_forecast_lasso2, aes(Time)) +
  ...
  labs(x = "Time", y = "GDP") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for Lasso Regression model")
```

```{r Q27}
# Accuracy for 12 Month - Lasso Regression model
forecast::accuracy(pred_CI_lasso2$point_predictions, test_df$GDP) %>% round(4)
```
...
ggplot(data_forecast_enet2, aes(Time)) +
  ...
  labs(x = "Time", y = "GDP") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for ElasticNet Regression model")
```
  
```{r Q27}
# Accuracy for 12 Month - ElasticNet Regression model
forecast::accuracy(pred_CI_enet2$point_predictions, test_df$GDP) %>% round(4)
```
...
ggplot(data_forecast_cart2, aes(Time)) +
  ...
  labs(x = "Time", y = "GDP") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for Decision Tree - CART model")
```

```{r Q27}
# Accuracy for 12 Month - Decision Tree - CART model
forecast::accuracy(pred_CI_cart2$point_predictions, test_df$GDP) %>% round(4)
```
...
ggplot(data_forecast_bagcart2, aes(Time)) +
  ...
  labs(x = "Time", y = "GDP") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for Bagged CART model")
```
  
```{r Q27}
# Accuracy for 12 Month - Bagged CART model
forecast::accuracy(pred_CI_bagcart2$point_predictions, test_df$GDP) %>% round(4)
```
...
ggplot(data_forecast_rf2, aes(Time)) +
  ...
  labs(x = "Time", y = "GDP") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for Random Forest model")
```

```{r Q27}
# Accuracy for 12 Month - Random Forest model
forecast::accuracy(pred_CI_rf2$point_predictions, test_df$GDP) %>% round(4)
```
...
ggplot(data_forecast_gbm2, aes(Time)) +
  ...
  labs(x = "Time", y = "GDP") +
  scale_color_manual(values = c(Actual = "red", Forecasted = "blue")) +
  theme_minimal() + ggtitle("12 Month Forecast vs. Actual Values for Gradient Boosting model")
```
  
```{r Q27}
# Accuracy for 12 Month - Gradient Boosting model
forecast::accuracy(pred_CI_gbm2$point_predictions, test_df$GDP) %>% round(4)
```
