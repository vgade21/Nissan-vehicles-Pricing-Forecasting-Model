...
train_ts <- ts(train_df$Semi_Price_Index)
test_ts <- ts(test_df$Semi_Price_Index)
tsdisplay(train_ts)
```
...
The p-value (0.7398) is greater than the significance level (0.05) for Semi_Price_Index
...
# tsdisplay(train_ts, main = 'Semi_Price_Index (training set) Jan 2013 - Sep 2019', xlab = 'Time') 
...
# tsdisplay(train_ts_trans, main = 'Box-Cox Transformation Semi_Price_Index with 𝝀= 1.999924', xlab = 'Time') 
```
...
# tsdisplay(diff(train_ts), main = '1st Differencing of Semi_Price_Index', xlab = 'Time') 
...
Semi_Price_Index <- train_ts

WPU03THRU15_Price_Index <- train_df[,"WPU03THRU15_Price_Index"]
WPU10_Price_Index <- train_df[,"WPU10_Price_Index"]
WPU1017_Price_Index <- train_df[,"WPU1017_Price_Index"]

# Fit a linear model
lin_model <- tslm(Semi_Price_Index ~ WPU03THRU15_Price_Index + WPU10_Price_Index + WPU1017_Price_Index)
summary(lin_model)
...
Semi_Price_Index <- train_ts
exogenous <- train_df[, c("WPU03THRU15_Price_Index", "WPU10_Price_Index", "WPU1017_Price_Index")]
# Convert the exogenous variables to a matrix
exogenous_matrix <- as.matrix(exogenous)

# Fit the auto.arima model with xreg
ARIMAX_model<- auto.arima(Semi_Price_Index, xreg = exogenous_matrix) 
...
xreg_vars <- test_df[, c("WPU03THRU15_Price_Index", "WPU10_Price_Index", "WPU1017_Price_Index")]
...
data_forecast_ARIMAX <- data.frame(Time = test_df$DATE,
                   Actual = test_df$Semi_Price_Index,
...
data_forecast_VAR <- data.frame(Time = test_df$DATE,
                   Actual = test_df$Semi_Price_Index,
                   Forecasted = fit_forecast_VAR$forecast$Semi_Price_Index)
```
...
ggplot(data_forecast_lin, aes(Time)) +
  ...
  labs(x = "Time", y = "Semi_Price_Index") +
...
forecast::accuracy(fit_forecast_lin, test_df$Semi_Price_Index) %>% round(4)
```
...
forecast::accuracy(fit_forecast_ARIMAX, test_df$Semi_Price_Index) %>% round(4)
```
...
```{r Q27}
# Accuracy for 18 Month - Linear model
forecast::accuracy(fit_forecast_VAR$forecast$Semi_Price_Index, test_df$Semi_Price_Index) %>% round(4)
```
...
fit.lm <- train(Semi_Price_Index~., data=train_df[, -1], method="lm", metric="RMSE", trControl=control)
print(fit.lm)
summary(fit.lm)
...
# Ridge (Regression)
set.seed(seedNum)
fit.ridge <- train(Semi_Price_Index~., data=train_df[, -1], method="ridge", metric="RMSE", trControl=control)
print(fit.ridge)
summary(fit.ridge)
...
set.seed(seedNum)
fit.lasso <- train(Semi_Price_Index~., data=train_df[, -1], method="lasso", metric="RMSE", trControl=control)
print(fit.lasso)
summary(fit.lasso)
...
set.seed(seedNum)
fit.enet <- train(Semi_Price_Index~., data=train_df[, -1], method="enet", metric="RMSE", trControl=control)
print(fit.enet)
summary(fit.enet)
...
set.seed(seedNum)
fit.cart <- train(Semi_Price_Index~., data=train_df[, -1], method="rpart", metric="RMSE", trControl=control)
print(fit.cart)
summary(fit.cart)
...
set.seed(seedNum)
fit.bagcart <- train(Semi_Price_Index~., data=train_df[, -1], method="treebag", metric="RMSE", trControl=control)
print(fit.bagcart)
summary(fit.bagcart)
...
set.seed(seedNum)
fit.rf <- train(Semi_Price_Index~., data=train_df[, -1], method="rf", metric="RMSE", trControl=control)
print(fit.rf)
summary(fit.rf)
...
set.seed(seedNum)
fit.gbm <- train(Semi_Price_Index~., data=train_df[, -1], method="xgbTree", metric="RMSE", trControl=control, verbose=F, verbosity = 0)
...
fit.ridge2 <- train(Semi_Price_Index~., data=train_df[, -1], method="glmnet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.ridge2)
print(fit.ridge2)
summary(fit.ridge2)
...
fit.lasso2 <- train(Semi_Price_Index~., data=train_df[, -1], method="glmnet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.lasso2)
print(fit.lasso2)
summary(fit.lasso2)
...
fit.enet2 <- train(Semi_Price_Index~., data=train_df[, -1], method="enet", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.enet2)
print(fit.enet2)
summary(fit.enet2)
...
fit.cart2 <- train(Semi_Price_Index~., data=train_df[, -1], method="rpart", metric="RMSE", tuneGrid=grid, trControl=control)
plot(fit.cart2)
print(fit.cart2)
summary(fit.cart2)
...
fit.bagcart2 <- train(Semi_Price_Index ~ ., data = train_df[, -1], method = "treebag",  metric = "RMSE", trControl = control, nbagg = 106)
#plot(fit.bagcart2)
print(fit.bagcart2)
...
# Perform hyperparameter tuning
fit.rf2 <- train(Semi_Price_Index ~ ., data = train_df[, -1], method = "rf", metric = "RMSE", trControl = control, 
...
# Perform hyperparameter tuning
fit.gbm2 <- train(Semi_Price_Index ~ ., data = train_df[, -1], method = "xgbTree", metric = "RMSE", trControl = control, verbose = FALSE, verbosity = 0, tuneGrid = hyperparameters)
plot(fit.gbm2)
print(fit.gbm2)
summary(fit.gbm2)
...
residuals <- test_df$Semi_Price_Index - point_predictions
...
forecast::accuracy(pred_CI_ridge2$point_predictions, test_df$Semi_Price_Index) %>% round(4)
```
...
forecast::accuracy(pred_CI_lasso2$point_predictions, test_df$Semi_Price_Index) %>% round(4)
```
...
forecast::accuracy(pred_CI_enet2$point_predictions, test_df$Semi_Price_Index) %>% round(4)
```
...
forecast::accuracy(pred_CI_cart2$point_predictions, test_df$Semi_Price_Index) %>% round(4)
```
...
forecast::accuracy(pred_CI_bagcart2$point_predictions, test_df$Semi_Price_Index) %>% round(4)
```
...
forecast::accuracy(pred_CI_rf2$point_predictions, test_df$Semi_Price_Index) %>% round(4)
```
...
forecast::accuracy(pred_CI_gbm2$point_predictions, test_df$Semi_Price_Index) %>% round(4)
```
